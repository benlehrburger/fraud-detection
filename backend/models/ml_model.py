    ...
    def engineer_advanced_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'card_number' in df.columns and 'timestamp' in df.columns:
            try:
                ...
                df['transactions_last_hour'] = df_temp.groupby('card_number').rolling('1H').size().reset_index(level=0, drop=True)
                df['transactions_last_day'] = df_temp.groupby('card_number').rolling('1D').size().reset_index(level=0, drop=True)
            except Exception as e:
                logger.warning(f"Could not calculate velocity features: {str(e)}")
        ...
        if 'amount' in df.columns and 'card_number' in df.columns:
            try:
                ...
                df['amount_std_7d'] = df.groupby('card_number')['amount'].transform(
                    lambda x: x.rolling(window=min(7, len(x)), min_periods=1).std()
                ).fillna(0)

                # Deviation from personal average
                df['amount_deviation'] = abs(df['amount'] - df['amount_mean_7d']) / (df['amount_std_7d'] + 1)
            except Exception as e:
                logger.warning(f"Could not calculate amount pattern features: {str(e)}")
